...
import { normalizeSootioUrl, normalizeTmdbLanguage, parseAudioLanguage, parseBooleanSetting, parseEnglishStreamMode, parseMdblistLists, parseMovieReleaseMode, parseShowAddDefaultMode, parseStreamProviderUrls, parseTraktLists } from '../config.js'
...
export async function uiRoutes(app: FastifyInstance) {
  ...
  app.post('/ui/settings-data', async (req, reply) => {
    ...
    if (typeof body.tmdbLanguage === 'string') {
      const language = normalizeTmdbLanguage(body.tmdbLanguage)
      setSetting('tmdbLanguage', language)
